...
def data_to_pic(dat, sz=SZ, min_s=MIN_STD, max_s=MAX_STD, s_factor=STD_FAC):
    ...
    #print("wrapper data fname, sz, min_s, max_s,s_factor ----> ",fname, sz, min_s, max_s, s_factor)
...
def partially_coherent_disarray(pic, reach, coherence, grain):
    ...
    #while templ:
     #  try:
      #     if fiducialDOGScaleSpace.next():
       #        print("I am in fiducialDOGScaleSpace")
        #   else:
         #      templ = False
       #except Exception as e:
        #   templ = False
    #print("fiducialDOGScaleSpace------------>> ",fiducialDOGScaleSpace.next())
    (h, w) = pic.fatFiducialDataPlane.shape
